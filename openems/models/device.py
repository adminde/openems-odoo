...
class Device(models.Model):
    ...
    _sql_constraints = [
        ...
        (
            "unique_stock_production_lot_id",
            "unique(stock_production_lot_id)",
        ...
        if "name" in vals:
            for record in self:
                if record.id and record.name != vals["name"]:
                    ...
                    if exists:
                        # Another device already carries the intended new name
                        raise exceptions.UserError(
                            _(
                                "The name '%(name)s' is already in use or does not "
                    ...
                    raise exceptions.UserError(
                        _("The name of the device cannot be changed after creation.")
                    )
    ...
    def _generate_unique_setup_password(self):
        ...
        while not is_unique:
            ...
            setup_password = "-".join(
                [raw_password[i : i + 4] for i in range(0, len(raw_password), 4)]
            )
    ...
    def _check_setup_password_format(self):
        for record in self:
            ...
            if not re.match(
                ...
                record.setup_password,
            ):
                raise ValidationError(
                    _("The device ID must be formatted as XXXX-XXXX-XXXX-XXXX")
                )
    ...
    def _check_api_key_uniqueness(self):
        for record in self:
            if record.apikey:
                ...
                # Saved records have an int id; exclude self from the search
                ...
                if existing:
                    raise ValidationError(
                        _(
                            "The API key already exists and must be unique. "
...
class Alerting(models.Model):
    ...
    sum_state_last_notification = fields.Datetime(
        string="Last SumState notification sent"
    )
    ...
    def _compute_user_role(self):
        for rec in self:
            ...
            # the user, a search would span every user's roles. The device is
            # bound as a default so the closure does not capture the loop var.
            device_role = rec.user_id.device_role_ids.filtered(
                lambda role, device=rec.device_id: role.device_id == device
            )
