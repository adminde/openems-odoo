...
class ProductionLot(models.Model):
    ...
    device_id = fields.Many2one(
        "openems.device", compute="_compute_device_id", inverse="_inverse_device_id"
    )
    ...
    def _inverse_device_id(self):
        if len(self.device_ids) > 0:
            if len(self.device_id.stock_production_lot_id) > 0:
                raise ValueError(
                    "A serial number has already been assigned to the device"
                )
